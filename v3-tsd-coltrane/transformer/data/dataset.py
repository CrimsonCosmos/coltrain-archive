...
class MIDIDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pad_id = getattr(self.tokenizer, 'pad_id', getattr(self.tokenizer, 'EVENT_PAD', 0))
        if len(seq) < self.seq_len:
            seq = seq + [pad_id] * (self.seq_len - len(seq))
...
class PreTokenizedDataset(Dataset):
    ...
    def __getitem__(self, idx):
        seq = self.data[idx][:self.seq_len].astype(np.int64)
...
def create_pretrain_dataloader(
    ...
    If groups_path is provided (JSON list mapping sequence index -> song group),
    uses song-group-aware splitting to prevent train/val leakage from transpositions.
    """
    ...
    if groups_path and Path(groups_path).exists():
        ...
        val_count = max(1, int(len(unique_groups) * val_ratio))
        val_groups = set(unique_groups[:val_count])
        train_indices = [i for i, g in enumerate(groups) if g not in val_groups]
        val_indices = [i for i, g in enumerate(groups) if g in val_groups]
        print(f"   Song-group split: {len(unique_groups) - val_count} train / {val_count} val groups")
    else:
        # Random split
